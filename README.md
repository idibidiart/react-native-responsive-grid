...
_The "grid" construct is extended beyond its archetypal form as a 2D Rectilinear Grid and generalized as a pair of components, namely, Row and Column, that can be used to compose a tree of UI Views. That is to say, a Row View (Row for short) may contain one or more Column Views (Column for short) each of which may contain one or more Rows, which in turn may contain one or more Columns, and so on. Both Rows and Columns can be styled using predictable, percentage-based width, height, padding and margins. Rows can be aligned vertically (along main axis) and horizontally (along cross axis) inside Columns, and Columns can be aligned horizontally (along main axis) and vertically (along cross axis) inside Rows. [Additionally, the multiple lines created by wrapped Row content may be aligned vertically relative to the parent Column.] This gives us the ability to build the entire UI component tree (or an individual component's subtree) using a consistent, repeatable and nestable pattern, one that has dynamic --not only static-- responsiveness and adaptivity built into it._
